...
class Toggle extends React.Component {
  ...
  internalSetState = (changes, callback) => {
    this.setState(currentState => {
      // const changedObject = typeof changes === 'function' ? changes(currentState) : changes;
      // const reducedChanges = this.props.stateReducer(currentState, changedObject) || {}
      // return Object.keys(reducedChanges).length ? reducedChanges : null

      return [changes].map(change => typeof change === 'function' ? change(currentState) : change)
                      .map(changedObject => this.props.stateReducer(currentState, changedObject) || {})
                      .map(reducedChanges => Object.keys(reducedChanges).length ? reducedChanges : null)[0]
    }, callback)
  }
  reset = () =>
